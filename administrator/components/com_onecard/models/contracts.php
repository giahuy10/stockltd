 ...
 * @since  1.6
 */
class OnecardModelContracts extends JModelList
{
/**
	* Constructor.
	*
...
'ncc','a.`ncc`',
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Filtering ncc
		$this->setState('filter.ncc', $app->getUserStateFromRequest($this->context.'.filter.ncc', 'filter_ncc', '', 'string'));
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__onecard_contract` AS a');
		...
		//Filtering ncc
		$filter_ncc = $this->state->get("filter.ncc");
		if ($filter_ncc)
		{
			$query->where("a.`ncc` = '".$db->escape($filter_ncc)."'");
		}
...
");
	...
	public function getItems()
	{
		...
		foreach ($items as $oneItem) {
			...
			if (isset($oneItem->ncc)) {
				$values = explode(',', $oneItem->ncc);
			...
			$oneItem->ncc = !empty($textValue) ? implode(', ', $textValue) : $oneItem->ncc;
