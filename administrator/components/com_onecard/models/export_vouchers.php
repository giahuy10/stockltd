 ...
 * @since  1.6
 */
class OnecardModelExport_vouchers extends JModelList
{
/**
	* Constructor.
	*
...
'created','a.`created`',
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Filtering event
		$this->setState('filter.event', $app->getUserStateFromRequest($this->context.'.filter.event', 'filter_event', '', 'string'));
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__onecard_export_voucher` AS a');
		...
		//Filtering event
		$filter_event = $this->state->get("filter.event");
		if ($filter_event)
		{
			$query->where("a.`event` = '".$db->escape($filter_event)."'");
		}
