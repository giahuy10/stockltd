 ...
 * @since  1.6
 */
class OnecardModelOrder_vouchers extends JModelList
{
/**
	* Constructor.
	*
...
'price','a.`price`',
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Filtering order_id
		$this->setState('filter.order_id', $app->getUserStateFromRequest($this->context.'.filter.order_id', 'filter_order_id', '', 'string'));
		

		// Filtering voucher_id
		$this->setState('filter.voucher_id', $app->getUserStateFromRequest($this->context.'.filter.voucher_id', 'filter_voucher_id', '', 'string'));
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__onecard_order_voucher` AS a');
		...
		//Filtering voucher_id
		$filter_voucher_id = $this->state->get("filter.voucher_id");
		if ($filter_voucher_id)
		{
			$query->where("a.`voucher_id` = '".$db->escape($filter_voucher_id)."'");
		}
 ...
 a.`order_id` like $search  || 
 a.`voucher_id` like $search  || 
 a.`quantity` like $search  || 
 a.`price` like $search 
");
			}
		}
	...
	public function getItems()
	{
		...
		foreach ($items as $oneItem) {
			...
			if (isset($oneItem->voucher_id)) {
				$values = explode(',', $oneItem->voucher_id);
			...
			$oneItem->voucher_id = !empty($textValue) ? implode(', ', $textValue) : $oneItem->voucher_id;
