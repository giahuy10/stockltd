...
$query
    ->select(array('a.voucher as voucher_id','c.title as voucher_name','d.title as brand', 'count(*) as quantity', 'c.input_price as price', 'c.type', 'c.expired' ,'c.unit as unit','a.type as type_code' ))
    ...
    if ($date_from)
	 ...
	 $query->group($db->quoteName('a.voucher'))
...
<table class="table table-bordered" id="resultsTableExportClient">
	...
	<tr class="noExl">
			...
			<td><select name="unit" onchange="this.form.submit()">
					<option value="">-- Lọc --</option>
					<option value="1" <?php if ($type == 1) echo "selected"?>>NCC</option>
					<option value="2" <?php if ($type == 2) echo "selected"?>>OneCard</option>
	...
	<?php foreach ($results as $result) {?>
		<tr>
			...
			<td><?php if ($result->type_code == 2) echo "OneCard"; else echo "NCC"?></td>
