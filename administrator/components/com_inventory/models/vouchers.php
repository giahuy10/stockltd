 ...
 * @since  1.6
 */
class InventoryModelVouchers extends JModelList
{
/**
	* Constructor.
	*
	...
	*/
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Filtering merchent
		$this->setState('filter.exported', $app->getUserStateFromRequest($this->context.'.filter.exported', 'filter_exported', '', 'string'));
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__inventory_voucher` AS a');
		...
		$query->join('LEFT', '#__categories AS `merchent` ON `merchent`.id = a.`merchent`');
		...
		// Filtering merchent
		//$filter_exported = $this->state->get("filter.exported");
		
		
		$filter_exported = $this->state->get("filter.exported");

		if ($filter_exported !== null && !empty($filter_exported))
		{
			$query->where("a.`exported` = '".$db->escape($filter_exported)."'");
		}
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm(
			'com_inventory.vouchers', 'vouchers',
			array('control' => 'jform',
				'load_data' => $loadData
			)
		);
